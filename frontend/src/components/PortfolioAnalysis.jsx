...
const GoalBasedAnalysis = ({ portfolio, goals }) => {
  ...
  const [simulationPeriod, setSimulationPeriod] = useState(60); // 60개월
  ...
  const calculateOptimalAllocation = () => {
    ...
    if (timeToGoal <= 12) { // 단기 (1년 이하)
      stockAllocation = 0.2;
      bondAllocation = 0.3;
      cashAllocation = 0.5;
    } else if (timeToGoal <= 60) { // 중기 (1-5년)
      stockAllocation = 0.5;
      bondAllocation = 0.4;
      cashAllocation = 0.1;
    } else { // 장기 (5년 이상)
  ...
  const calculateProbability = () => {
    ...
    const probability = projectedFinalValue >= currentGoal.amount ? 
    ...
    <div className="bg-white rounded-lg p-6 shadow-sm">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div className="p-4 bg-blue-50 rounded-lg">
          ...
          >
            {goals.map(goal => (
              <option key={goal.id} value={goal.id}>{goal.name} - {goal.amount.toLocaleString()}원</option>
            ))}
          </select>
        </div>
        ...
        <div style={{ height: '300px' }}>
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={projectionData}>
              ...
              <XAxis 
                dataKey="month" 
                label={{ value: '경과 개월', position: 'insideBottom', offset: -5 }} 
              />
              <YAxis 
                tickFormatter={(value) => `${(value / 10000).toFixed(0)}만`}
                label={{ value: '포트폴리오 가치 (원)', angle: -90, position: 'insideLeft' }} 
              />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div>
          ...
          <div className="bg-gray-100 p-6 rounded-lg h-full flex flex-col justify-center items-center">
            <div className="text-5xl font-bold mb-4 text-blue-600">{goalProbability}%</div>
      ...
      <div className="mt-8 bg-blue-50 p-4 rounded-lg">
        ...
        <ul className="space-y-2 text-gray-700">
          <li>• 월 투자금액을 {monthlyContribution < 1000000 ? '증가' : '유지'}하여 목표 달성 가능성을 높이세요.</li>
          <li>• 포트폴리오 구성을 추천 자산 배분과 일치시켜 위험 대비 수익을 최적화하세요.</li>
